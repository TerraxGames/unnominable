...
void Shader::add_shader_path(ShaderType shader_type, std::vector<const char *> const &file_paths_vector) {
...
bool Shader::compile_and_link() {
    ...
    for (const auto shader_type : keys) {
        ...
        glAttachShader(this->shader_program, this->shader_objects.at(shader_type));
    }
    ...
    if (!this->success) {
        ...
        SDL_LogCritical(SDL_LOG_CATEGORY_APPLICATION, "Shader program linking failed:\n%s\n", this->info_log);
        return false;
    }
...
void Shader::set_uniform_bool(const std::string &name, bool value) {
    glUniform1i(glGetUniformLocation(this->shader_program, name.c_str()), static_cast<int>(value));
}
...
bool Shader::compile_shader_pipe(ShaderType shader_type) {
    ...
    for (int i = 0; i < shader_paths.size(); i++) {
        ...
        if (shader_file.fail()) {
            SDL_LogError(SDL_LOG_CATEGORY_APPLICATION, "Failed to find shader %s\n", file_path.c_str());
            continue;
        }
    ...
    this->shader_objects.insert(std::pair<ShaderType, GLuint>(shader_type, glCreateShader(shader_type)));
    ...
    if (!this->success) {
        ...
        SDL_LogCritical(SDL_LOG_CATEGORY_APPLICATION, "Shader compilation failed:\n%s\n", this->info_log);
        return false;
    }
